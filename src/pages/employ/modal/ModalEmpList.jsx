...
const ModalLecture = (props) => {
    ...
    useEffect(() => {
        ...
        if (props.id !== null && props.id !== "") {
            console.log("ModalLecture> pros.id=" + props.id);

            lecrmDetail(props.id);
            return () => {
                setSelinfo({});
            }    
        }
    ...
    // 강의실 정보 상세보기
    const lecrmDetail = (id) => {

        let params = new URLSearchParams();
        params.append("lecrm_id", id);

        axios
            .post("/employ/stdInfoDetail.do", params)
            .then((res) => {
                setSelinfo(res.data.selinfo);
                console.log("stdInfoDetail> res.data.selinfo : " + JSON.stringify(res.data.selinfo));
            })
            .catch((err) => {
                alert(err.message);
            });
    ...
    const postHandler = (action) => {
        ...
        if (action !== "D") {
            let checkresult = commonjs.nullcheck([
                { inval: selinfo.lecrm_name, msg: "회사명을 입력해 주세요." },
                { inval: selinfo.lecrm_size, msg: "입사일을 입력해 주세요." },
            ]);
            if (!checkresult) return;
        }
        let params = new URLSearchParams(selinfo);
        params.append("std_id", props.id);
